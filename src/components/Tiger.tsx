...
const Tiger: React.FC<NodeProps> = (props: NodeProps) => {
  ...
  const xy = new Animated.ValueXY(zeroDelta);
  const { x: dx, y: dy } = xy;
  ...
  const { panHandlers } = PanResponder.create({
    ...
    onPanResponderRelease: (e, gestureState) => {
      ...
      let step =
        gestureState.dx >= 1.5 * spacing ||
          gestureState.dx <= -1.5 * spacing ||
          gestureState.dy >= 1.5 * vSpacing ||
          gestureState.dy <= -1.5 * vSpacing
          ? 2
          : 1;
      console.log("got step", step);
      //how to reject pan if gestureState value is more than 120

      let nextPosition = findNextPosition(initialPosition, direction, step);
      //while finding nex position check validity
      console.log(
        "got direction to ",
        nextPosition,
        " from ",
      ...
      if (moveAllowed) {
        props.moveTiger(initialPosition, nextPosition, step);
      } else {
  ...
  const panXY = {
    ...
    onClick: async () => {
      ...
      //     sound: soundObject,
      //     status,
      //   } = await Audio.Sound.createAsync(tigerAudio, { shouldPlay: true });
  ...
  return (
    <>
      <Symbol id="tiger" viewBox="0 0 60 60">
        <G fill="none" >
          <Path
